...
// Handle of the currently connected host, or BLE_HS_CONN_HANDLE_NONE when idle.
// Tracked so pairing-mode can drop the active link.
static uint16_t s_conn_handle = BLE_HS_CONN_HANDLE_NONE;

// One-shot timer that, a few seconds after connecting, asks for a generous
// supervision timeout so the link rides out brief RF glitches instead of
// dropping with reason 0x208. Deferred because (a) Apple requires peripherals to
// wait >=5 s before requesting a parameter update and (b) firing it during
// pairing collides with the encryption LL procedure (which dropped tvOS).
...
static void conn_param_timer_cb(void *arg) {
    ...
    struct ble_gap_upd_params cp = {
        .itvl_min = 12,             /* 12 * 1.25 ms = 15 ms (Apple requires >= 15 ms) */
        .itvl_max = 12,             /* == min (Apple allows the 15/15 ms case) */
        .latency = 0,
        .supervision_timeout = 500, /* 500 * 10 ms = 5 s (Apple max 6 s); tolerant of misses */
    };
    int rc = ble_gap_update_params(s_conn_handle, &cp);
    ESP_LOGI(TAG, "delayed conn params update; rc=%d", rc);
}

static void conn_param_timer_arm(void) {
    if (s_conn_param_timer == NULL) {
        const esp_timer_create_args_t a = { .callback = conn_param_timer_cb, .name = "conn_param" };
        if (esp_timer_create(&a, &s_conn_param_timer) != ESP_OK) {
            return;
        }
    }
    esp_timer_stop(s_conn_param_timer);          // no-op if not running
    esp_timer_start_once(s_conn_param_timer, 5000000);  // 5 s after connect
}

// Multi-device support: each slot advertises under its own static-random
// identity address, so every paired host keeps an independent bond and
// Fn+F1..F4 switches between them. The active slot is persisted in NVS.
#define DEVICE_SLOTS  4
static uint8_t s_slot;
static bool    s_slot_loaded;

static void slot_load(void) {
    if (s_slot_loaded) return;
    s_slot_loaded = true;
    nvs_handle_t h;
    if (nvs_open("kbd", NVS_READONLY, &h) == ESP_OK) {
        uint8_t v;
        if (nvs_get_u8(h, "slot", &v) == ESP_OK && v < DEVICE_SLOTS) {
            s_slot = v;
        }
        nvs_close(h);
    }
...
static void slot_save(void) {
    nvs_handle_t h;
    if (nvs_open("kbd", NVS_READWRITE, &h) == ESP_OK) {
        nvs_set_u8(h, "slot", s_slot);
        nvs_commit(h);
        nvs_close(h);
    }
}
...
static void slot_addr(uint8_t slot, uint8_t out[6]) {
    uint8_t mac[6] = {0};
    esp_efuse_mac_get_default(mac);   // mac[0] = OUI MSB
    out[0] = (uint8_t)(mac[5] + 1 + slot);
    out[1] = mac[4];
    out[2] = mac[3];
    out[3] = mac[2];
    out[4] = mac[1];
    out[5] = (uint8_t)(mac[0] | 0xC0);
}
...
esp_err_t esp_hid_ble_gap_adv_init(uint16_t appearance, const char *device_name)
{
     ...
     * The board has no display or keypad, so use "Just Works" pairing
     * (no passkey, no MITM). Bonding + LE Secure Connections still encrypt the
     * link, which is what the HID service requires. */
...
nimble_hid_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_CONNECT:
        ...
        if (event->connect.status == 0) {
            ...
            /* Proactively start pairing/encryption: HID input reports require an
             * encrypted link, and not every host initiates this on its own. */
            rc = ble_gap_security_initiate(event->connect.conn_handle);
            ESP_LOGI(TAG, "security initiate; rc=%d", rc);

            /* Arm the deferred parameter update (generous supervision timeout). */
            conn_param_timer_arm();

            /* Deliberately do NOT request a connection-parameter update here.
             * tvOS reacts badly to a peripheral-initiated update right after
             * connect (the request collides with / times out against its own
             * setup -- seen as conn-update status 0x22A then 0x208, then a
             * disconnect with reason 0x208) and drops the link. The central
             * already picks sensible HID parameters, so we let it manage them;
             * macOS/Android negotiate fine on their own too. */
        }
        return 0;
    case BLE_GAP_EVENT_DISCONNECT:
        ESP_LOGI(TAG, "disconnect; reason=%d", event->disconnect.reason);
        s_conn_handle = BLE_HS_CONN_HANDLE_NONE;
        if (s_conn_param_timer) esp_timer_stop(s_conn_param_timer);
        return 0;
    ...
    case BLE_GAP_EVENT_NOTIFY_TX:
        /* Fires once per HID report (~60/s while the trackball moves);
         * intentionally not logged so it cannot stall the report cadence. */
        return 0;
...
esp_err_t esp_hid_ble_gap_adv_start(void)
{
    ...
    /* Give the active device slot its own static-random identity so each paired
     * host keeps a separate bond. If setting the address fails we fall back to
     * the public address (single-device behaviour) so basic pairing still works. */
    uint8_t own_addr_type = BLE_OWN_ADDR_PUBLIC;
    slot_load();
    uint8_t addr[6];
    slot_addr(s_slot, addr);
    rc = ble_hs_id_set_rnd(addr);
    if (rc == 0) {
        own_addr_type = BLE_OWN_ADDR_RANDOM;
    } else {
        ESP_LOGW(TAG, "ble_hs_id_set_rnd (slot %u) rc=%d; using public address", s_slot, rc);
    }
...
{
    if (slot >= DEVICE_SLOTS) {
        return ESP_ERR_INVALID_ARG;
    }
    slot_load();
    s_slot = slot;
    slot_save();
    ESP_LOGI(TAG, "switching to device slot %u", slot);

    if (s_conn_handle != BLE_HS_CONN_HANDLE_NONE) {
        // Drop the current host; the disconnect path re-advertises, and
        // esp_hid_ble_gap_adv_start() applies the new slot's identity.
        int rc = ble_gap_terminate(s_conn_handle, BLE_ERR_REM_USER_CONN_TERM);
        ESP_LOGI(TAG, "switch: terminate conn rc=%d", rc);
    } else {
        ble_gap_adv_stop();              // stop advertising as the old slot
        esp_hid_ble_gap_adv_start();     // re-advertise with the new identity
    }
    return ESP_OK;
}

esp_err_t esp_hid_ble_gap_pairing_reset(void)
{
    // Forget every stored bond so a previously-paired host no longer blocks a
    // fresh pairing (this is the device-side half of "forget the device on the
    // host first" from the README).
    int rc = ble_store_clear();
    ESP_LOGI(TAG, "pairing reset: ble_store_clear rc=%d", rc);

    if (s_conn_handle != BLE_HS_CONN_HANDLE_NONE) {
        // Drop the current host; the disconnect path restarts advertising, just
        // like a host-initiated disconnect does.
        rc = ble_gap_terminate(s_conn_handle, BLE_ERR_REM_USER_CONN_TERM);
        ESP_LOGI(TAG, "pairing reset: terminate conn rc=%d", rc);
    } else {
        // Idle: (re)start advertising so the 180 s window is fresh and open.
        rc = esp_hid_ble_gap_adv_start();
        ESP_LOGI(TAG, "pairing reset: adv_start rc=%d", rc);
    }
    return ESP_OK;
}
#endif
