...
// Enter pairing mode: clear all stored bonds, drop the active link (if any), and
// (re)start advertising so a new host can pair from scratch.
esp_err_t esp_hid_ble_gap_pairing_reset(void);

// Switch to BLE device slot `slot` (0-based): drop the current host and
// (re)advertise under that slot's own identity, so each slot keeps a separate
